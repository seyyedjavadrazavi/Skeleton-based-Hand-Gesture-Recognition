...
read_file = pd.read_csv (r'annotations_revised_training.txt')
data = read_file.values.tolist()
col = read_file.columns.values.tolist()
data.insert(0, col)

with open('annotations_revised_training.csv', 'w', newline='') as out_file:
...
labels = pd.read_csv('annotations_revised_training.csv')
...
for i in range(1, 109, 1):
    ...
    for label in crnt_label.iterrows():
        ...
        with open('training_data.csv', 'a', newline='') as out_file:
            writer = csv.writer(out_file)
            for line in data:
